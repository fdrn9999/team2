...
- **사용자 관점**: "`price` 컬럼명을 `cost`로 변경하자."
- **시스템 처리 (트랜잭션)**:
    1. `UPDATE COL_META SET COL_NM = 'cost' WHERE COL_NM = 'price';`
    2. `UPDATE TBL_SAMPLE SET DATA_JSON = JSON_REMOVE(JSON_SET(DATA_JSON, '$.cost', JSON_VALUE(DATA_JSON, '$.price')), '$.price');` (실제 데이터의 Key도 변경)
